...
.zarf-src/         vendored upstream zarf packages/ tree (generated by `uds run vendor`, gitignored)
```
...
`.zarf-src` already exists for a different source version, `uds run vendor` re-vendors it
automatically.
