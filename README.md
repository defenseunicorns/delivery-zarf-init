...
uds run cluster:remove                           # remove the deployed init package
uds run test:all                                 # health checks + agent mutation + values assertions
uds run cluster:cleanup                          # tear down the uds-k3d cluster and artifacts
...
switching packages needs `uds run cluster:remove` first (or a fresh cluster). The default `CLUSTER_NAME` is
...
Publishable PRs must advance the package version for every affected flavor.
